...
bool Frame::getNext(std::ifstream &file, short format, Points *points)
{
	FrameData data;
	short numberOfDataBytes = data.getNumberOfDataBytes(format);
	char buffer[numberOfDataBytes];
	...
	// End of file...
	if (fileSize - (position + Frame::NUMBER_OF_HEADER_BYTES) < Frame::NUMBER_OF_HEADER_BYTES + numberOfDataBytes)
	{
	...
	do
	{
		...
		file.read(buffer, numberOfDataBytes);
		data.x = (buffer[FrameData::X_BYTE] << 8) + buffer[FrameData::X_BYTE + 1];
		data.y = (buffer[FrameData::Y_BYTE] << 8) + buffer[FrameData::Y_BYTE + 1];
		data.laserOn = !getBit(buffer[FrameData::STATUS_BYTE], FrameData::LASER_ON_BIT);
...
bool Frame::isLastPoint(char *buffer)
{
	return getBit(buffer[FrameData::STATUS_BYTE], FrameData::LAST_POINT_BIT);
}
